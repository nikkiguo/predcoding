...
def get_train_set(dataset_name):
    if dataset_name == "MNIST":
        ...
        return torchvision.datasets.CIFAR10(root='CIFAR_train', train=True, download=True, transform=None)
...
def get_test_set(dataset_name):
    if dataset_name == "MNIST":
        ...
        return torchvision.datasets.CIFAR10(root='CIFAR_test', train=False, download=True, transform=None)
...
def img_to_np(dataset_name, img):
    if dataset_name == "CIFAR10":
        return np.array(img).transpose((2, 0, 1)).reshape([3, 32, 32]) / 255.0 # reorder from (height, width, channels) to (channels, height, width) and scale pixels to float format -> 3D numpy array
    else:
        return np.array(img).reshape([784]) / 255.0


def get_imgs(dataset_name, dataset):
    imgs = np.array([img_to_np(dataset_name, dataset[i][0]) for i in range(len(dataset))])
...
def plot_imgs(dataset_name, img_batch, save_path):
    img_batch = img_batch.detach().cpu().numpy()
    batch_size = img_batch.shape[1]
    dim = nearest_square(batch_size)

    if dataset_name == "CIFAR10":
        imgs = [np.transpose(img_batch[:, i], (1, 2, 0)) for i in range(dim ** 2)] # Convert back to HxWxC for display
    else:
